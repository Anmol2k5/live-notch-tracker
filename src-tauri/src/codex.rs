    ...
    match resp {
        ...
        Err(ureq::Error::Status(code @ (401 | 403), r)) => {
            ...
            let head: String = r
                ...
                .take(160)
                .collect();
            println!("LOG: {}", &format!("codex: usage endpoint HTTP {code}: {head}"));
        ...
        match load_credential() {
            ...
            Some(cred) => match fetch_usage(&cred) {
                Ok(v) => {
                    ...
                    println!("LOG: {}", &format!("codex: usage reply has no windows (top-level keys {keys:?}), falling back to the rollout"));
                ...
                Err(LiveErr::RateLimited(secs)) => {
                    ...
                    println!("LOG: {}", &format!("codex: usage endpoint returned 429, retrying in {secs}s"));
                }
                Err(LiveErr::Other(e)) => {
                    println!("LOG: {}", &format!("codex: live read failed ({e}), falling back to the rollout"));
...
fn broadcast(app: &AppHandle, snap: UsageSnapshot) {
    ...
    *st.codex.lock().unwrap_or_else(|e| e.into_inner()) = snap.clone();
    persist(&snap);
...
pub fn start(app: AppHandle) {
    std::thread::spawn(move || {
        {
            let st = app.state::<AppState>();
            let snap = st.codex.lock().unwrap_or_else(|e| e.into_inner()).clone();
            let _ = app.emit("codex", &snap);
        }
        if !present() {
...
mod tests {
    ...
    #[test]
    fn test_snapshot_from_rollout() {
        let text = r#"
{"timestamp":"2026-09-12T10:00:00Z","type":"event_msg","payload":{"type":"token_count","rate_limits":{"primary":{"used_percent":25.0,"window_minutes":300,"resets_in_seconds":18000},"plan_type":"pro"}}}
        "#;
        let (windows, recorded, plan) = snapshot_from_rollout(text).unwrap();
        assert_eq!(windows.len(), 1);
        assert_eq!(windows[0].id, "primary");
        assert_eq!(windows[0].used, 0.25);
        assert_eq!(windows[0].label, "5h limit");
        assert_eq!(plan.unwrap(), "pro");
