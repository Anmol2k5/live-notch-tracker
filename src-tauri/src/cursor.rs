...
fn broadcast(app: &AppHandle, snap: UsageSnapshot) {
    ...
    *st.cursor.lock().unwrap_or_else(|e| e.into_inner()) = snap.clone();
    persist(&snap);
...
pub fn start(app: AppHandle) {
    std::thread::spawn(move || {
        {
            let st = app.state::<AppState>();
            let snap = st.cursor.lock().unwrap_or_else(|e| e.into_inner()).clone();
            let _ = app.emit("cursor", &snap);
        }
        if !present() {
        ...
        loop {
            let prev = {
                let st = app.state::<AppState>();
                let s = st.cursor.lock().unwrap_or_else(|e| e.into_inner()).clone();
                s
            };
...
mod tests {
    ...
    fn test_parse_summary_basic() {
        let data = json!({
            "billingCycleEnd": "2026-10-12T00:00:00Z",
            "membershipType": "pro",
            "isUnlimited": false,
            "individualUsage": {
                "plan": {
        ...
        let (windows, note) = parse_summary(&data);
